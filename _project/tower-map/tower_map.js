window.onload = function(){
    ...
    var decimal2string = function (lat, lng){
        var res = ['', ''];
        res[0] += geolib.decimal2sexagesimal(lat);
        res[0] += lat > 0 ? ' E' : ' W';
        res[1] += geolib.decimal2sexagesimal(lng);
        res[1] += lng > 0 ? ' N' : ' S';
        return res;
    }
    ...
    option = {
        ...
        tooltip: {
            ...
            formatter: function (tower) {
                var res = '';
                res += '<div style="border-bottom: 1px solid rgba(0, 0, 0, .3);'
                    + ' font-size: 18px;padding-bottom: 7px;margin-bottom: 7px">'
                    + tower.name + '</div>';
                var lat_lng = decimal2string(tower.value[0], tower.value[1]);
                res += '经：' + lat_lng[0] + '<br />'
                    + '纬：' + lat_lng[1] + '<br />';
                if (tower.value[2])
            ...
            textStyle: {
                color: map_color.light,
                fontSize: 12
            },
            borderWidth: 1,
            borderColor: map_color.light,
            padding: [15, 10],
            extraCssText: 'box-shadow: 0 0 5px rgba(0, 0, 0, 0.3);'
                            + 'cursor: default',
            position: function (point, params, dom, rect, size) {
                /* In the least_area, t_pos won't be changed more than twice */
                var least_area = 15;
                /* Set the offset of tooltip */
                var mapSize = size.viewSize;
                var tooltipSize = size.contentSize;
                var margin = 50;
                var offset_x = point[0] > (mapSize[0] - tooltipSize[0]) ? 
                        -(tooltipSize[0] + margin) : margin;
                var offset_y = -tooltipSize[1] / 2;
